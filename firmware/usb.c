...
static USBControlState control_state = {0};
...
void usb_handle_ep0()
{
	if((USB->EP0R & USB_EP_CTR_RX) != 0)
	{
		...
		usb_set_endpoint(&USB->EP0R, USB_EP_RX_VALID, USB_EP_CTR_TX | USB_EP_CTR_RX);
	}
	...
	if((USB->EP0R & USB_EP_CTR_TX) != 0)
	{
		...
		if(control_state.transfer.len > 0)
		{
			if(control_state.transfer.len > control_state.transfer.sent)
			{
				usb_prepare_transfer(&control_state.transfer, &USB->EP0R, 
				(volatile uint8_t*)&EP0_buffer_tx, 64, &BTable[0].COUNT_TX);
			}
		}
...
{
	// A setup packet must stop any ongoing transfers
	usb_copy_memory((volatile uint16_t*)pkt, (volatile uint16_t*)&control_state.setup, sizeof(USBSetupPacket));
	control_state.transfer.len = 0;
	if((pkt->req_type & 0x0F) == 0)
	{
		...
		switch(pkt->req)
		{
			case 0x06: // Get descriptor request
				switch(pkt->desc_type)
				{
					case 0x01: // Device descriptor
					{
						uint16_t* desc = (uint16_t*)usb_get_descriptor();
						usb_copy_memory(desc, (uint16_t*)EP0_buffer_tx, sizeof(USBDescriptorDevice));
					...
					{
						uint16_t len;
						uint8_t* desct = (uint8_t*)usb_get_config_descriptor(&len);
						control_state.transfer.buffer = desct;
						control_state.transfer.sent = 0;
						control_state.transfer.len = MIN(len, pkt->len);
						usb_prepare_transfer(&control_state.transfer, &USB->EP0R, 
							(volatile uint8_t*)&EP0_buffer_tx, 64, &BTable[0].COUNT_TX);
					} break;
					case 0x06: // Device qualifier descriptor (we "stall" to signify that we do high speed)
			...
			case 0x05: // Set address request
				...
				// We do send an empty reply (with the "previous" ADDR)
				BTable[0].COUNT_TX = 0;
				usb_set_endpoint(&USB->EP0R, USB_EP_TX_VALID, USB_EP_TX_VALID);
				break;
		}
...
static void usb_add_to_descriptor(const uint8_t* data, uint16_t* offset)
{
	uint16_t len = data[0];
...
uint8_t* usb_get_config_descriptor(uint16_t* len)
{
	...
	{
		uint16_t offset = 0;
		usb_add_to_descriptor((uint8_t*)&ConfigDescriptor, &offset);
		usb_add_to_descriptor((uint8_t*)&InterfaceDescriptors[0], &offset);
		usb_add_to_descriptor((uint8_t*)&EndpointDescriptors[0], &offset);
		usb_add_to_descriptor((uint8_t*)&EndpointDescriptors[1], &offset);
	}

	*len = sizeof(ConfigurationBuffer);
	return (uint8_t*)ConfigurationBuffer;
}
...
{
	*txbuffer_count = MIN(buffer_size, trans->len - trans->sent);
	if(*txbuffer_count > 0)
	{
		// There's still data to send
		usb_copy_memory((uint16_t*)(trans->buffer + trans->sent), (uint16_t*)buffer, *txbuffer_count);
		trans->sent += *txbuffer_count;
